...
async function handleGET(request: Request) {
  ...
  if (USE_MOCK) {
    const statistics = mockGetPersonalStatistics(startDate, endDate);
  ...
  const url = `${BE_BASE}/api/statistics/personal${queryString ? `?${queryString}` : ""}`;
  ...
  // Parse response - backend may wrap in ApiResponse format
