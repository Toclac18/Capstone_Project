...
public class ImportServiceImpl implements ImportService {
  ...
  public ImportListResponse list(int page, int pageSize, String q, String status) {
    ...
    var filtered = all.stream()
        .filter(j -> q == null || q.isBlank()
            ...
            || (j.getCreatedBy() != null && j.getCreatedBy().toLowerCase()
            .contains(q.toLowerCase()))
            || (j.getStatus() != null && j.getStatus().name().toLowerCase()
            .contains(q.toLowerCase())))
  ...
  @Override
  public ImportDetailResponse create(MultipartFile file) {
    String orgId = null;
    try {
      ...
      if (attrs != null) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
          ...
          try {
            ...
            // email được set ở claim "email" khi tạo token → dùng extractEmail
            // (đã có sẵn trong JwtUtil)
            String emailFromJwt = jwtUtil.extractEmail(
                claims); // :contentReference[oaicite:0]{index=0}
            if (emailFromJwt != null && !emailFromJwt.isBlank()) {
              Optional<Organization> org = orgRepo.findByEmail(emailFromJwt);
              orgId = org.isPresent() ? String.valueOf(org.get().getId()) : emailFromJwt;
            }
          } catch (Exception ignored) {
            // token không hợp lệ hoặc thiếu claim → sẽ fallback bên dưới
          }
        }
      }

      // 3) Quyết định createdBy để lưu/audit
      String createdByValue = (orgId != null && !orgId.isBlank())
          ? orgId
          : "readeeplatform@gmail.com";

      // 4) Tạo job & lưu
      ImportJob job = new ImportJob();
      job.setFileName(file.getOriginalFilename());
      job.setCreatedAt(OffsetDateTime.now());
      job.setCreatedBy(createdByValue);
      job.setStatus(ImportStatus.PROCESSING);
      repo.save(job);

      // 5) Gọi xử lý
      byte[] data = file.getBytes();
      processor.process(job.getId(), data, createdByValue);
