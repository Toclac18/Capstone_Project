...
import type { PersonalDocumentStatistics, StatisticsQueryParams } from "@/types/statistics";
import { StatisticsFilters } from "./_components/StatisticsFilters";
import { SummaryCards } from "./_components/SummaryCards";
import { DocumentUploadsChart } from "./_components/DocumentUploadsChart";
import { ActivityCharts } from "./_components/ActivityCharts";
import { BreakdownCharts } from "./_components/BreakdownCharts";
...
export default function StatisticsPage() {
  ...
  const [statistics, setStatistics] = useState<PersonalDocumentStatistics | null>(null);
  ...
  useEffect(() => {
    ...
    const loadStatistics = async (params?: StatisticsQueryParams) => {
      try {
        ...
        setState("loading");
        setError(null);
        const data = await getPersonalStatistics(params);
        if (!isMounted) return;
        setStatistics(data);
        setState("success");
      } catch (err: any) {
      ...
      {state === "success" && statistics && (
        <div className="mt-6 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <DocumentUploadsChart data={statistics.documentUploads} />
            <ActivityCharts statistics={statistics} />
          </div>

          <BreakdownCharts
            statusBreakdown={statistics.statusBreakdown}
