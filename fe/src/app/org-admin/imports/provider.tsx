...
import { fetchImports, type ImportListResponse } from "@/services/orgAdmin-imports";
...
export function useImportHistory() {
  ...
  if (!ctx) throw new Error("useImportHistory must be used within ImportHistoryProvider");
  return ctx;
}

export default function ImportHistoryProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    fetchImports(filters).then(d => { if (mounted) setData(d); }).finally(() => mounted && setLoading(false));
  ...
  return (
    <ImportHistoryCtx.Provider value={{ data, loading, filters, setFilters, gotoPage }}>
      {children}
