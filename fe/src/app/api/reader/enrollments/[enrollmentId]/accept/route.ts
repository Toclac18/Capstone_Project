...
async function handlePOST(
  ...
  if (USE_MOCK) {
    ...
    return jsonResponse("Invitation accepted successfully", {
      status: 200,
  ...
  const upstream = await fetch(
    `${BE_BASE}/api/reader/enrollments/${enrollmentId}/accept`,
    {
      method: "POST",
      headers: fh,
      cache: "no-store",
...
export const POST = (...args: Parameters<typeof handlePOST>) =>
  withErrorBoundary(() => handlePOST(...args), {
    context: "api/reader/enrollments/[enrollmentId]/accept/route.ts/POST",
  });
